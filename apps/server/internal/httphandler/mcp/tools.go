...
// tools returns the list of MCP tool definitions. Input schemas are built via
// the small DSL in schema.go. Descriptions are prescriptive — they tell the
// model *when* to use each tool, not just what it does.
func tools() []mcpTool {
	return []mcpTool{
		{
			Name:        "berth_deploy",
		...
		},
		{
			...
			InputSchema: schema(map[string]any{
				...
				"files":         prop("object", "Map of relative file paths to file contents (replaces all files). Max 100 files, 10MB total."),
		...
		// ── Secret tools ─────────────────────────────────────────────
		{
			...
			InputSchema: schema(map[string]any{
				...
				"value":       prop("string", "Secret value (encrypted at rest, never returned)"),
				"description": prop("string", "Human-readable description of the secret (optional)"),
				"global":      prop("boolean", "If true, the secret is available to all users (optional, default false)"),
			}, "name", "value"),
		},
		{
		...
		},
		{
			...
			InputSchema: schema(map[string]any{
				...
				"global": prop("boolean", "If true, delete a global secret (optional, default false)"),
			}, "name"),
		},
		{
			Name: "berth_guide",
